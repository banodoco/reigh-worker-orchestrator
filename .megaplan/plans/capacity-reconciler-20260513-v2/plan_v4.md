...
The remaining gate feedback is schema/invariant precision, not a new approach. The reconciler remains pool-scoped for capacity and locking, while route keys are used only for demand attribution, backoff, and audit. This revision resolves the last two contradictions:

1. Authoritative mode has the exact-one-row pool intent invariant. Shadow mode is allowed to write one pool summary row per observer per local cycle because shadow replicas do not acquire leases; shadow rows carry observer metadata and audit queries dedupe/compare by observer.
2. Route backoff keys are never nullable. Pool-floor or buffer spawns use a sentinel route key (`__pool_floor__`) so PostgreSQL can enforce a real primary key or unique constraint on `(pool, route_key)`.
...
3. `worker_capacity_intents` is the pool audit table. In authoritative mode it writes exactly one pool summary row per pool per reconciler cycle. In shadow mode it writes one row per observer per local cycle, tagged with `observer_id` and `observation_id` so duplicate shadow observers are explicit rather than accidental.
4. `worker_capacity_route_backoffs.route_key` is `NOT NULL`; use `__pool_floor__` for pool-floor/buffer spawn actions that are not attributable to queued route demand.
5. `excluded_from_capacity_control` remains a hard boundary. Excluded live-test or harness workers are not counted, spawned for, cancelled, or terminated by capacity reconciliation; periodic cleanup still sees them through the existing cleanup path.
6. The rollout is three deployable slices with separate gates: shadow, authoritative cutover, and final cleanup. Grep-zero deletion gates apply only to the final cleanup slice.
7. Phase A intentionally reorders lifecycle probing before legacy capacity actions so shadow observation is post-lifecycle and pre-capacity, as the brief requires.
8. Authoritative mode resolves the ACT/RECORD contradiction in favor of invariant 7: it records the admitted pool intent before side effects, then updates that same row and route-backoff state with action outcomes after side effects.
...
2. Add `observer_id text` and `observation_id uuid DEFAULT gen_random_uuid()` to `worker_capacity_intents` so shadow rows from multiple replicas can be distinguished and deduped without pretending local `cycle_id` is globally unique.
3. Use `worker_capacity_intents` as the pool audit trail. Authoritative mode writes exactly one summary row per pool per reconciler cycle, normally with `route_key=NULL`, `desired_capacity` equal to the pool-level desired capacity, and observed counts equal to pool totals.
4. Shadow mode writes one summary row per observer per local cycle. It does not acquire leases and therefore must not claim global exact-one-row semantics.
5. Add `worker_capacity_route_backoffs` keyed by `(pool, route_key)` with `route_key text NOT NULL`, `consecutive_spawn_failures`, `next_spawn_allowed_at`, `updated_at`, and last failure/success metadata.
6. Define a single sentinel constant for pool-floor/buffer actions, `POOL_FLOOR_ROUTE_KEY='__pool_floor__'`, and enforce uniqueness with `PRIMARY KEY (pool, route_key)` or an equivalent unique constraint. Do not use nullable route keys in the backoff table.
7. Add an `orchestrator_leases` table keyed by `pool`, not route, with `owner_id`, `expires_at`, and `updated_at`.
8. Add lease acquire/release RPCs with TTL semantics. Do not use advisory locks; the lease must survive multiple Supabase HTTP calls while RunPod side effects execute.
9. Keep `system_logs.valid_source_type` unchanged; shadow and authoritative audit logs continue to use `source_type='orchestrator_gpu'`.
...
4. Add a cheap schema verification step confirming `worker_capacity_intents`, `worker_capacity_route_backoffs`, `orchestrator_leases`, the backoff primary key, and the shadow observer columns exist.

### Step 3: Add database helpers (`gpu_orchestrator/database.py`)
**Scope:** Medium
1. Add helpers to insert one pool intent row, update that same row with action outcome totals, read latest authoritative pool intent by `(pool, shadow=false)`, read shadow rows by `(pool, observer_id, cycle_id)`, and acquire/release pool leases.
2. Add helpers to read/update route backoff state by `(pool, route_key)`, with all pool-floor calls normalized to `__pool_floor__` before database access.
...
3. `ObservedPool` must carry included worker states, excluded worker states, route demands, pool-level active/idle/busy/spawning counts, route-stale counts, pool, cycle id, observer id, observation id, snapshot time, and selected route contract metadata.
...
4. When no route key is available, use `__pool_floor__` only for pool-floor/buffer spawn attribution and emit a warning/audit tag for demand that cannot be route-attributed.
...
5. Allocate any `Spawn` action budget across route demands after computing the single pool spawn budget. Prioritize uncovered non-backoff route demand, then pool-floor/buffer spawns attributed to `__pool_floor__`.
...
2. Generate a stable `observer_id` at process/reconciler construction time and a fresh `observation_id` for every reconciliation. Include both in intent rows and shadow system log metadata.
3. Shadow mode runs observe -> plan -> admit -> record -> shadow log and never calls RunPod, mutates workers, acquires leases, or updates route backoff state.
4. Shadow mode may produce multiple rows for the same pool and local cycle when multiple replicas are running. This is expected; audit queries must group by observer or dedupe by observation metadata.
5. Authoritative mode acquires the pool lease before final admit/record/act. If the pool lease is unavailable, record/log a lease-suppressed result and skip side effects.
6. `admit()` suppresses only route-attributed `Spawn` actions whose normalized `(pool, route_key)` has `next_spawn_allowed_at > clock.now()`. Backoff on one route must not suppress other route demand in the same pool.
7. `record_pre_act()` must insert the single authoritative pool summary intent before any authoritative side effect. `record_outcome()` updates that row after spawn success/failure or cancel/terminate results.
8. On route-attributed spawn failure, update `worker_capacity_route_backoffs` for that normalized `(pool, route_key)` with `consecutive_spawn_failures += 1` and `next_spawn_allowed_at = now + min(2**N * 30s, 600s)`. On successful spawn for that key, reset the route counter to 0.
...
3. For shadow mode, group by `observer_id` and `observation_id`, and provide a deduped pool/cycle view for operators. Do not assume shadow rows are globally unique by `(pool, cycle_id)`.
4. For authoritative mode, report exactly one row per pool/cycle under the pool lease.
5. Report pool desired, effective capacity, legacy spawns/terminations, route backoff suppressions, cancel recommendations, lease suppressions, summary counters, and route-count fallback cases.
...
2. Report shadow vs authoritative cycles, divergence counts, pool-level intent stability, shadow observer counts/deduped views, route backoff windows, route-key fallback counts, latest adoption intents, summary counters, and lease-suppressed actions.
...
3. Pool-floor backoff uses normalized route key `__pool_floor__`; duplicate pool-floor backoff rows cannot be inserted for the same pool.
4. Pool capacity bounds: multiple selected route keys cannot multiply `min_active_gpus`, `machines_to_keep_idle`, or `max_active_gpus`; the pool desired capacity is capped once.
5. Pool single-writer: two authoritative reconciler instances cannot both execute side effects for the same pool, even if they are acting on different route-attributed demand.
6. Cancel hysteresis: test both `2 * orchestrator_poll_sec` and `valid_until` branches at pool level.
7. Route-stale worker exclusion: a route-stale worker does not satisfy fresh selected-pool capacity.
8. Cutover adoption: pre-existing spawning workers with no authoritative intent are adopted and not cancelled.
9. Excluded worker boundary: excluded spawning/active workers are not counted or mutated by capacity reconciliation.
10. CycleSummary integration: reconciler-driven spawns, pending-spawn cancels, idle terminations, failed spawns, backoff suppressions, and lease suppressions appear in result counters and `CycleSummary` as appropriate.
11. Replay fixture: synthetic or captured 2026-05-13 incident state produces no oscillation and no premature cancel.
...
6. Authoritative pool intent invariant: exactly one `worker_capacity_intents` summary row is written per pool per authoritative cycle.
7. Shadow audit invariant: multiple shadow observers may write rows for the same pool/local cycle, but each row has `observer_id` and `observation_id`, and audit dedupe logic can distinguish them.
...
5. `python -m pytest -q`
6. Shadow-slice gate: schema verified, shadow has no side effects, observation ordering test passes, shadow rows include observer metadata, divergence SQL/debug output includes pool summaries and route backoff details.
7. Authoritative-slice gate: record-before-act, pool lease, idempotency, route backoff uniqueness, pool capacity bounds, summary counters, adoption, and replay tests pass.

...
We will:
1. Compute a sha256 hash of the *semantic* inputs that determine venv state — `uv.lock` contents, the `--python` value, the `--extra` flags, and the `uv` binary's `--version` output.
...
### Landmine awareness
- The hash MUST cover all four inputs. Missing any one means a config drift goes silently undetected.
...
- `--locked` stays on the actual `uv sync` invocation when it runs (lockfile-drift backstop preserved).
...
3. **Confirm** that the script's working directory at this point is the repo root (the `cd "$WORKDIR"` at line 241), where `uv.lock` and `.venv/` live.
...
# must be re-synced. We hash the *contents* of uv.lock (not its mtime) plus
# the literal CLI inputs and the uv binary's own version. Comparison is on
# the file contents of the sentinel — never on an exit code — because
# proxy checks like `git submodule status … >/dev/null` can return 0 on
# broken states (see commit 134e3ec).
...
Notes on choices:
1. **`sha256sum` over `md5sum`** — collisions matter here (a silent miss means a stale venv); sha256sum is in coreutils on the RunPod Ubuntu image (apt installs `coreutils` by default; the script already calls `sha256sum`-class tooling implicitly via `uv`).
...
4. **`echo "uv-lock-sha256: MISSING"`** — degrades to a deterministic value if `uv.lock` is missing (which would be a packaging bug); the subsequent `uv sync` will then surface the real error. We never want a missing lockfile to silently produce the same hash twice.
5. **`update_worker_phase "deps_verified"` (line 347 today) is left unchanged** — it sits *after* the `fi`, so the orchestrator phase signal fires on both the skip path and the sync path.
...
def test_rendered_startup_script_gates_uv_sync_on_inputs_sentinel():
    ...
    # All four inputs must contribute to the hash.
...
4. **Manual / out-of-band (info only):** on the next real worker boot, confirm `/tmp/worker_startup_*.log` shows the `⏭️  Skipping uv sync` line on a warm boot with no lock change, and the `✅ uv sync complete; .venv/.sync-inputs refreshed` line on a cold boot or after editing `uv.lock`.
